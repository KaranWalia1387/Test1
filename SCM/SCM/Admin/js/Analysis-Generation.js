...
//    GenerationUsageAnalysis.setcookie(tz.toString());
//    // Expire in one year
...
function PiechartCommon(mode, drawDayType, drawHourType) {
    ...
    $('#time-GenerationChart').html("<b>" + title + "</b>");
    $('#time-TopGenerationChart').html("<b>" + title + "</b>");
    $('#time-GenerationTrendChart').html("<b>" + title + "</b>");
    if (drawDayType == true) {
    ...
    GenerationChart(mode);
    TopCustomersChart();
    GenerationTrendChart(mode);
...
function WeekendAndWeekDayChart(mode, caseId) {
    ...
    weekEndDayChartTable = GenerationTable.value.Tables[0];
    ...
    var toolTipArray = ['Day Type: ', 'Solar in kwh: '];
...
function PeakNonPeakChart(mode, caseId) {
    ...
    peakNonPeakChartTable = GenerationTable.value.Tables[1];
    ...
    var toolTipArray = ['Hour Type: ', 'Solar in kwh: '];
    BindPieSeries('div-PeakNonPeakChart', chart2, seriesname, title, seriesType, toolTipArray);
}

function GenerationChart(mode, caseId) {
    var generationChart;
    generationChart = GenerationTable.value.Tables[2];

    chart3 = new Array();
    var seriesname = '';

    $.map(generationChart.Rows, function (obj, i) {
        var name = '';
        switch (mode) {
            case 'y':
                name = obj.Year;
    ...
    if (generationChart.Rows.length > 3) {
        var divwidth = generationChart.Rows.length * 200;
        $('#div-GenerationChart').attr('style', 'width:' + (divwidth) + 'px!important;float:left!important');
        $('#h3-GenerationChart').attr('style', 'width:' + (divwidth) + 'px!important');
    }
    else {
        $('#div-GenerationChart').attr('style', 'width:822px!important;float:left!important');
        $('#h3-GenerationChart').attr('style', 'width:822px!important');
    }

    var title = 'Solar Analysis';
    BindColumnSeriesLocal('div-GenerationChart', chart3, seriesname, title, 'Solar (kwh)');
}

function TopCustomersChart(mode, caseId) {
    var customerChartTable, customerTypeTable;
    customerChartTable = GenerationTable.value.Tables[3];
    customerTypeTable = GenerationTable.value.Tables[5]
    ...
    if (customerChartTable.Rows.length > 10) {
        ...
        $('#div-TopGenerationChart').attr('style', 'width:' + (divwidth) + 'px!important;float:left!important');
        $('#h3-TopGenerationChart').attr('style', 'width:' + (divwidth) + 'px!important');
    }
    else {
        $('#div-TopGenerationChart').attr('style', 'width:822px!important;float:left!important');
        $('#h3-TopGenerationChart').attr('style', 'width:822px!important');
    }
    
    var title = 'Top Solar Customers';
    BindColumnSeriesWithCategoriesLocal('div-TopGenerationChart', chart4, seriesname, categories, title, 'Solar in kwh');
}

function GenerationTrendChart(mode, caseId) {
    var generationChart, regionTable, categoryTable;
    generationChart = GenerationTable.value.Tables[4];
    regionTable = GenerationTable.value.Tables[6]
    categoryTable = GenerationTable.value.Tables[7];
    ...
    if (categoryTable.Rows.length > 4) {
        ...
        $('#div-GenerationTrendChart').attr('style', 'width:' + (divwidth) + 'px!important;float:left!important');
        $('#h3-GenerationTrendChart').attr('style', 'width:' + (divwidth) + 'px!important');
    }
    else {
        $('#div-GenerationTrendChart').attr('style', 'width:822px!important;float:left!important');
        $('#h3-GenerationTrendChart').attr('style', 'width:822px!important');
    }

    var title = 'Solar Trend Analysis';
    var toolTipArray = ['Solar Period: ', 'Solar Region: ', 'Solar (kWh): '];
    BindLineSeriesLocal('div-GenerationTrendChart', chart5, seriesname, title, 'Solar (kWh)', toolTipArray);
}
...
function DrillDown(period, x, y, currentSeriesName, seriesType) {
    ...
    switch (currentSeriesName) {
        case 'Year':
            ...
            GenerationTable = GenerationUsageAnalysis.GetData(mode, monthFrom, monthTo, city, zip, ddlAccountType, hourType, dayType);
        ...
        case 'Month':
            ...
            $('#txtDateTo').val(dayTo);
            GenerationTable = GenerationUsageAnalysis.GetData(mode, dayFrom, dayTo, city, zip, ddlAccountType, hourType, dayType);
            PiechartCommon(mode, true, true);
            break;
        case 'Day':
            mode = 'h';
            ...
            GenerationTable = GenerationUsageAnalysis.GetData(mode, hourDay, hourDay, city, zip, ddlAccountType, hourType, dayType);
            PiechartCommon(mode, true, true);
            break;
    }
}
...
function DrillUp(name, x, y, currentSeriesName) {
    ...
    switch (currentSeriesName) {
        case 'Month':
            ...
            GenerationTable = GenerationUsageAnalysis.GetData(mode, $('#txtDateFrom').val(), convertlocaltoutc($('#txtDateTo').val()), city, zip, ddlAccountType, hourType, dayType);
            PiechartCommon(mode, true, true);
            break;
        case 'Day':
            mode = 'm';
...
$(document).ready(function () {
    ...
    GenerationTable = GenerationUsageAnalysis.GetData(mode, $('#txtDateFrom').val(), convertlocaltoutc($('#txtDateTo').val()), '', '', '', '', hourType, dayType);
...
function SetHourType(type) {
    ...
    switch (mode) {
        case 'y':
            GenerationTable = GenerationUsageAnalysis.GetData(mode, $('#txtDateFrom').val(), $('#txtDateTo').val(), city, zip, ddlAccountType, hourType, dayType);
            break;
        case 'm':
            GenerationTable = GenerationUsageAnalysis.GetData(mode, monthFrom, monthTo, city, zip, ddlAccountType, hourType, dayType);
            break;
...
function submit() {
    ...
    SetFilterValues();
    GetDatesDifferenceInDays();
    GenerationTable = GenerationUsageAnalysis.GetData(mode, dtFrom, dtTo, city, zip, ddlAccountType, hourType, dayType);
    PiechartCommon(mode, true, true);
}
    ...
    try {
        $('#' + id).highcharts({
            ...
            tooltip: {
                useHTML: true,
                formatter: function () {
                    return '<tr><td style="color: {series.color}">Solar Period: </td>' +
                            '<td style="text-align: right"><b>' + this.point.name + '</b></td></tr>' + '</br>' +
                            '<tr><td style="color: {series.color}">Solar (kWh): </td>' +
                            '<td style="text-align: right"><b>' + Math.round(this.y) + '</b></td></tr>';
                }
            },
            dataLabels: {
