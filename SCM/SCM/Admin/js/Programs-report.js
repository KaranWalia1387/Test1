...
function LoadGrid() {
    try {
        ...
        source = {
            datatype: "array",
            datafields: [
                    { name: 'DatePrograms', type: 'date', sorttype: "date", datefmt: "mm/dd/yyyy" },
        ...
        $("#jqxgrid").jqxGrid({
            ...
            columns:
               [
                  { text: 'Date', dataField: 'DatePrograms', width: '25%', hidden: false, cellsformat: "MM/dd/yyyy" },
                  ...
                  { text: 'Total Requests', dataField: 'TotalEnrollment', width: '25%' },//, cellsrenderer: imagerenderer
               ]
        });
    }
    catch (e) {
...
function PiechartCommon(mode, caseId) {
    ...
    if (piechart.Rows.length > 0) {
        ...
        $('#nodata_div').show();
        $('#nodata_div1').show();
        $('#nodata_div').html('<font color="Red">No Program  Data Available</font>');
        $('#nodata_div1').html('<font color="Red">No Program  Data Available</font>');
...
function FillChartData(mode) {
    try {
        ...
        yaxis = 'Number of programs';

        $.map(databindtogrid, function (obj, i) {
            processed_json.push({
                name: obj.DatePrograms.split('T')[0].slice(5).split('-')[0] + '/' + obj.DatePrograms.split('T')[0].slice(5).split('-')[1],
...
$(document).ready(function () {
    ...
    //$("#btnExcelExport1").click(function () {
    //    try {
    ...
    //        //else
    //        //    LoadChildGrid();
    //        $("#" + gridid).jqxGrid('exportdata', 'xls', 'Programs Report');
            ...
            success: function (data) {
                ...
                if (result != null) {
                    ...
                    if (databindtogrid.length == 0)
                        // document.getElementById('btnExcelExport1').disabled = true;
                        for (var i = 0; i < databindtogrid.length; i++) {

                            var dateEnds = new Date(databindtogrid[i]["DatePrograms"]);
                            databindtogrid[i]["DatePrograms"] = (dateEnds.getMonth() + 1) + '/' + dateEnds.getDate() + '/' + dateEnds.getFullYear();
                        }
                    //for pdf
